...
(() => {
  ...
  class Nv00Gnb {
    constructor(el) {
      this.els = {
        ...
        innerWrapLoopingStartEl: el.querySelector('.nv00-gnb__inner-wrap-looping--start'),
        innerWrapLoopingEndEl: el.querySelector('.nv00-gnb__inner-wrap-looping--end'),
        ...
        l1MenuContainerEls: el.querySelectorAll('.nv00-gnb__l1-menu-container'),
        ...
        featureProductsThumbnailItemLinkEls: el.querySelectorAll('.nv00-gnb__featured-products-thumbnail-item-link'),
        utilityWrapEl: el.querySelector('.nv00-gnb__utility-wrap'),
        beforeLoginButtonEl: el.querySelector('.nv00-gnb__utility.before-login'),
        afterLoginButtonEl: el.querySelector('.nv00-gnb__utility.after-login'),
      ...
      this.isRtl = this.utils.isRtl;
      this.activeL0Menu = false;
      this.activeL1Menu = false;
      this.isSwiperInit = false;
      this.currentDevice = this.getCurrentDevice();
      this.addEventListener = this.utils.addEventListener;
      this.removeAllEventListeners = this.utils.removeAllEventListeners;
      this.isInteraction = [];
    ...
    l1MenuShowEventForDesktop(e) {
      if (e.type === 'keydown') {
        if (!(e.keyCode === KEY_CODE.ENTER) && !(e.keyCode === KEY_CODE.DOWN)) {
          //console.log('return');
          return;
        }
      }
      ...
      // if (e) {
      //   console.log(e.type, ': ', this.utils.closest(e.target, '.nv00-gnb__l0-menu').querySelector('.nv00-gnb__l0-menu-btn').innerText);
      // }

      const index = this.utils.closest(e.target, '.nv00-gnb__l0-menu').dataset.index;
      //console.log('index: ', index, ', ', this.isInteraction[index]);

      if (this.isInteraction[index].mouse) {
        //console.log('mouse 로 여는 중');
        return;
      }

      if (this.isInteraction[index].key) {
        //console.log('key 로 여는 중');
        return;
      }

      if (e.type === 'mouseenter') {
        this.isInteraction[index].mouse = true;
      }

      if (e.type === 'keydown') {
        this.isInteraction[index].key = true;
      }
      ...
      let l1Container = e.target.querySelector('.nv00-gnb__l1-menu-container');
      let el = e.target;

      if (e.target.classList.contains('nv00-gnb__l0-menu-btn')) {
        if (e.keyCode === KEY_CODE.ENTER || e.keyCode === KEY_CODE.DOWN) {
          l1Container = e.target.nextElementSibling;
          el = e.target.parentElement;
          el.classList.add('focus');
          setTimeout(() => {
            l1Container.querySelector('a, button').focus();
          }, 200);
      ...
      if (l1Container.classList.contains('left-side')) {
        l1Container.style.top = `${el.getBoundingClientRect().bottom - 24}px`;
        if (!this.utils.isRtl()) {
      ...
      } else if (l1Container.classList.contains('right-side')) {
        l1Container.style.top = `${el.getBoundingClientRect().bottom - 24}px`;
        if (!this.utils.isRtl()) {
          l1Container.style.right = `${this.els.el.getBoundingClientRect().right - this.els.innerWrapEl.getBoundingClientRect().right + 48}px`;
        } else {
      ...
      //console.log('l1ContainerHeight: ', l1ContainerHeight);
      l1Container.style.height = '0';
      l1Container.classList.add('show');

      requestAnimationFrame(() => {
        l1Container.style.height = `${l1ContainerHeight}px`;
        //console.log('after open:', l1Container.querySelector('.nv00-gnb__l1-menu-container-title').innerText);
        if (e.type === 'mouseenter') {
          this.isInteraction[index].mouse = false;
        }

        if (e.type === 'keydown') {
          setTimeout(() => {
            this.isInteraction[index].key = false;
          }, 200);
        }
      });

      window.sg.common.lazyLoad.setLazyLoadManually(l1Container);
      this.utils.setMobileFocusLoop(this.els.el);
    }

    l1MenuHideEventForDesktop(e) {
      //console.log('l1MenuHideEventForDesktop');
      if (e) {
        //console.log(e.type, ': ', this.utils.closest(e.target, '.nv00-gnb__l0-menu').querySelector('.nv00-gnb__l0-menu-btn').innerText);

        if (e.type === 'mouseleave' && e.target.matches(':hover')) {
          return;
        }
      ...
      if (e) {
        ...
        l1Container = this.els.l0MenuWrapEl.querySelector('.nv00-gnb__l1-menu-container.show');
        if (l1Container) {
          el = l1Container.parentElement;
        } else {
          return;
        }
      }
      ...
      l1Container.style.height = '0';
      l1Container.classList.remove('open');

      const handleTransitionend = () => {
        if (!l1Container.classList.contains('open')) {
          l1Container.removeAttribute('style');
          l1Container.classList.remove('show');
        }

        l1Container.removeEventListener('transitionend', handleTransitionend);
      };

      l1Container.addEventListener('transitionend', handleTransitionend);


      if (!this.els.el.querySelector('.nv00-gnb__l1-menu-container.open')) {
        this.els.dimWrapEl.classList.remove('show');
        this.els.el.classList.remove('show');
        this.utils.removeMobileFocusLoop();
      }
      el.querySelector('.nv00-gnb__l0-menu-btn').setAttribute('aria-expanded', 'false');
    }
    ...
    l1MenuShowEventForMobile(e) {
      ...
      [...this.els.l0MenuEls].forEach((l0) => {
        ...
        l0.querySelector('.nv00-gnb__l0-menu-btn, .nv00-gnb__l0-menu-link').setAttribute('tabindex', '-1');
      });
      this.els.userAccountWrapEl.style.opacity = '0';
      [...this.els.userAccountWrapEl.querySelectorAll('.nv00-gnb__user-account-link')].forEach((link) => {
        link.setAttribute('tabindex', '-1');
      });
      this.els.utilityWrapEl.style.opacity = '0';
      [...this.els.utilityWrapEl.querySelectorAll('.nv00-gnb__utility-btn, .nv00-gnb__utility-user-menu-link')].forEach((btn) => {
        btn.setAttribute('tabindex', '-1');
      });

      if (!this.utils.isRtl()) {
        this.els.userAccountWrapEl.style.right = '100%';
        this.els.utilityWrapEl.style.right = '100%';
        this.els.l0MenuWrapEl.style.right = '100%';
      } else {
        this.els.userAccountWrapEl.style.left = '100%';
        this.els.utilityWrapEl.style.left = '100%';
        this.els.l0MenuWrapEl.style.left = '100%';
      }
      ...
      this.utils.removeMobileFocusLoop();
      this.utils.setMobileFocusLoop(selectedL0El);
      this.els.searchWrapEl.removeAttribute('data-aria-hidden');
      this.els.searchWrapEl.removeAttribute('aria-hidden');
      ...
      if (el.querySelector('.nv00-gnb__featured-products-thumbnail-item-container') && !this.isSwiperInit) {
        ...
        this.swiper.on('slideChange', () => {
          window.sg.common.lazyLoad.setLazyLoadManually(el.querySelector('.nv00-gnb__featured-products-thumbnail-item-container'));
        });

        [...el.querySelectorAll('.nv00-gnb__featured-products-thumbnail-item-container .nv00-gnb__featured-products-thumbnail-item-link')].forEach((item, index) => {
          this.addEventListener(item, 'focus', () => {
            el.querySelector('.nv00-gnb__featured-products-thumbnail-item-wrap').scrollLeft = 0;
            if ((item.getBoundingClientRect().left < (el.querySelector('.nv00-gnb__featured-products-thumbnail-item-wrap').getBoundingClientRect().left + this.swiper.navigation.prevEl.getBoundingClientRect().width)) || (item.getBoundingClientRect().right > (el.querySelector('.nv00-gnb__featured-products-thumbnail-item-wrap').getBoundingClientRect().right - this.swiper.navigation.nextEl.getBoundingClientRect().width))) {
              this.swiper.slideTo(index);
            }
          });
        });

        this.swiper.navigation.prevEl.setAttribute('tabindex', '-1');
        this.swiper.navigation.prevEl.setAttribute('aria-hidden', 'true');
        this.swiper.navigation.nextEl.setAttribute('tabindex', '-1');
        this.swiper.navigation.nextEl.setAttribute('aria-hidden', 'true');
    ...
    l2MenuShowEventForMobile(e) {
      ...
      if (l1.classList.contains('show')) {
        ...
        if (this.activeL1Menu) {
          ...
          this.activeL1Menu = false;
          beforeActiveL1Menu.nextElementSibling.style.height = '0';
          setTimeout(() => {
        ...
        if ((this.activeL1Menu.nextElementSibling.nextElementSibling && !this.activeL1Menu.nextElementSibling.nextElementSibling.classList.contains('desktop-only')) || this.activeL1Menu.parentElement.nextElementSibling) {
          this.activeL1Menu.nextElementSibling.style.marginBottom = '12px';
        }
        requestAnimationFrame(() => {
          const l2MenuListHeight = this.activeL1Menu.nextElementSibling.getBoundingClientRect().height;
          requestAnimationFrame(() => {
            this.activeL1Menu.nextElementSibling.style.height = '0';
            requestAnimationFrame(() => {
              this.activeL1Menu.nextElementSibling.style.visibility = 'visible';
              this.activeL1Menu.nextElementSibling.style.height = `${l2MenuListHeight}px`;
              setTimeout(() => {
    ...
    bindEvents() {
      ...
      [...this.els.l0MenuEls].forEach((el) => {
        if (this.getCurrentDevice() === 'desktop') {
          ...
          if (el.querySelector('.nv00-gnb__l0-menu-link')) {
          ...
          if (el.querySelector('.nv00-gnb__l0-menu-btn')) {
            ...
            [...el.querySelectorAll('.nv00-gnb__l1-menu-btn')].forEach((l1) => {
              ...
              l1.addEventListener('keydown', this.l2MenuShowEventForMobile);
            });
          }
        }
      });
        ...
        el.addEventListener('keydown', (e) => {
          if (this.getCurrentDevice() === 'desktop') {
            if (e.keyCode === KEY_CODE.RIGHT || e.keyCode === KEY_CODE.DOWN) {
              let activeNextL1Menu = false;
              let nextL1Menu = el.parentElement.nextElementSibling;

              while (nextL1Menu) {
                if (!nextL1Menu.classList.contains('mobile-only')) {
                  activeNextL1Menu = nextL1Menu;
                  break;
                }

                nextL1Menu = nextL1Menu.nextElementSibling;
              }

              if (activeNextL1Menu) {
                activeNextL1Menu.querySelector('.nv00-gnb__l1-menu-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').nextElementSibling && this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').nextElementSibling.classList.contains('featured-products-thumbnail')) {
                this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').nextElementSibling.querySelector('.nv00-gnb__featured-products-thumbnail-item-link').focus();
              }
              e.preventDefault();
            } else if (e.keyCode === KEY_CODE.LEFT || e.keyCode === KEY_CODE.UP) {
              let activePrevL1Menu = false;
              let prevL1Menu = el.parentElement.previousElementSibling;

              while (prevL1Menu) {
                if (!prevL1Menu.classList.contains('mobile-only')) {
                  activePrevL1Menu = prevL1Menu;
                  break;
                }

                prevL1Menu = prevL1Menu.previousElementSibling;
              }

              if (activePrevL1Menu) {
                activePrevL1Menu.querySelector('.nv00-gnb__l1-menu-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l0-menu')) {
                this.utils.closest(el, '.nv00-gnb__l0-menu').querySelector('.nv00-gnb__l0-menu-btn').focus();
              }
              e.preventDefault();
            }
          }
      ...
      [...this.els.l2MenuLinkEls].forEach((el) => {
        el.addEventListener('keydown', (e) => {
          if (this.getCurrentDevice() === 'desktop') {
            if (e.keyCode === KEY_CODE.RIGHT || e.keyCode === KEY_CODE.DOWN) {
              let activeNextL2Menu = false;
              let nextL2Menu = el.parentElement.nextElementSibling;

              while (nextL2Menu) {
                if (!nextL2Menu.classList.contains('mobile-only')) {
                  activeNextL2Menu = nextL2Menu;
                  break;
                }

                nextL2Menu = nextL2Menu.nextElementSibling;
              }

              if (activeNextL2Menu) {
                activeNextL2Menu.querySelector('.nv00-gnb__l2-menu-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l2-menu-list').nextElementSibling) {
                this.utils.closest(el, '.nv00-gnb__l2-menu-list').nextElementSibling.nextElementSibling.querySelector('a, button').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').nextElementSibling) {
                this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').nextElementSibling.querySelector('a, button').focus();
              }

              e.preventDefault();
            } else if (e.keyCode === KEY_CODE.LEFT || e.keyCode === KEY_CODE.UP) {
              let activePrevL2Menu = false;
              let prevL2Menu = el.parentElement.previousElementSibling;

              while (prevL2Menu) {
                if (!prevL2Menu.classList.contains('mobile-only')) {
                  activePrevL2Menu = prevL2Menu;
                  break;
                }

                prevL2Menu = prevL2Menu.previousElementSibling;
              }

              if (activePrevL2Menu) {
                activePrevL2Menu.querySelector('.nv00-gnb__l2-menu-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l2-menu-list').previousElementSibling.previousElementSibling && this.utils.closest(el, '.nv00-gnb__l2-menu-list').previousElementSibling.previousElementSibling.classList.contains('nv00-gnb__l2-menu-list')) {
                this.utils.closest(el, '.nv00-gnb__l2-menu-list').previousElementSibling.previousElementSibling.querySelector('.nv00-gnb__l2-menu:last-of-type .nv00-gnb__l2-menu-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').previousElementSibling.classList.contains('nv00-gnb__l1-menu-wrap')) {
                this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').previousElementSibling.querySelector('.nv00-gnb__l2-menu-list:last-of-type .nv00-gnb__l2-menu:last-of-type .nv00-gnb__l2-menu-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l0-menu')) {
                this.utils.closest(el, '.nv00-gnb__l0-menu').querySelector('.nv00-gnb__l0-menu-btn').focus();
              }
      ...
      [...this.els.featureProductsThumbnailItemLinkEls].forEach((el) => {
        el.addEventListener('keydown', (e) => {
          if (this.getCurrentDevice() === 'desktop') {
            if (e.keyCode === KEY_CODE.RIGHT || e.keyCode === KEY_CODE.DOWN) {
              if (el.parentElement.nextElementSibling) {
                el.parentElement.nextElementSibling.querySelector('.nv00-gnb__featured-products-thumbnail-item-link').focus();
              }

              e.preventDefault();
            } else if (e.keyCode === KEY_CODE.LEFT || e.keyCode === KEY_CODE.UP) {
              if (el.parentElement.previousElementSibling) {
                el.parentElement.previousElementSibling.querySelector('.nv00-gnb__featured-products-thumbnail-item-link').focus();
              } else if (this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').previousElementSibling.classList.contains('l1-only')) {
                this.utils.closest(el, '.nv00-gnb__l1-menu-wrap').previousElementSibling.querySelector('.nv00-gnb__l1-menu:last-of-type .nv00-gnb__l1-menu-link').focus();
              }
      ...
      [...this.els.l1MenuContainerEls].forEach((el, index) => {
        el.parentElement.setAttribute('data-index', index);
        this.isInteraction.push({ 'mouse': false, 'key': false });

        const firstL1Menu = el.querySelector('.nv00-gnb__l1-menu-wrap.l1-only .nv00-gnb__l1-menu-list .nv00-gnb__l1-menu .nv00-gnb__l1-menu-link');
        if (firstL1Menu) {
          firstL1Menu.addEventListener('blur', () => {
            requestAnimationFrame(() => {
              if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__l1-menu-link')) {
        ...
        const lastL1Menu = el.querySelector('.nv00-gnb__l1-menu-wrap.l1-only:last-of-type .nv00-gnb__l1-menu-list:last-of-type .nv00-gnb__l1-menu:last-of-type .nv00-gnb__l1-menu-link');
        if (lastL1Menu) {
          lastL1Menu.addEventListener('blur', () => {
            requestAnimationFrame(() => {
              if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__l1-menu-link')) {
                this.l1MenuHideEventForDesktop();
              }
        ...
        const lastFeaturedMenu = el.querySelector('.nv00-gnb__l1-menu-wrap.featured-products-thumbnail .nv00-gnb__featured-products-thumbnail-item-wrap .nv00-gnb__featured-products-thumbnail-item:last-of-type .nv00-gnb__featured-products-thumbnail-item-link');
        if (lastFeaturedMenu) {
          lastFeaturedMenu.addEventListener('blur', () => {
            requestAnimationFrame(() => {
              if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__featured-products-thumbnail-item-link')) {
                this.l1MenuHideEventForDesktop();
              }
            });
          });
        ...
        const firstL2Menu = el.querySelector('.nv00-gnb__l1-menu-wrap.l1-l2-spread .nv00-gnb__l2-menu-list .nv00-gnb__l2-menu .nv00-gnb__l2-menu-link');
        if (firstL2Menu) {
          firstL2Menu.addEventListener('blur', () => {
            requestAnimationFrame(() => {
              if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__l2-menu-link')) {
                this.l1MenuHideEventForDesktop();
              }
            });
          });
        ...
        const lastL1L2SpreadMenu = el.querySelectorAll('.nv00-gnb__l1-menu-wrap.l1-l2-spread:not(.mobile-only)')[el.querySelectorAll('.nv00-gnb__l1-menu-wrap.l1-l2-spread:not(.mobile-only)').length - 1];
        if (lastL1L2SpreadMenu) {
          const lastL2Menu = lastL1L2SpreadMenu.querySelector('.nv00-gnb__l2-menu-list:last-of-type .nv00-gnb__l2-menu:last-of-type .nv00-gnb__l2-menu-link');
          if (lastL2Menu) {
            lastL2Menu.addEventListener('blur', () => {
              requestAnimationFrame(() => {
                if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__l2-menu-link')) {
                  this.l1MenuHideEventForDesktop();
                }
      ...
      this.els.backwardButtonEl.addEventListener('click', () => {
        this.utils.removeMobileFocusLoop();
        this.utils.setMobileFocusLoop(this.els.innerWrapEl);
        // 열려진 L0 메뉴 이동
        ...
        [...this.els.l0MenuEls].forEach((l0) => {
          ...
          l0.querySelector('.nv00-gnb__l0-menu-btn, .nv00-gnb__l0-menu-link').removeAttribute('tabindex');
        });
        ...
        this.els.l0MenuWrapEl.removeAttribute('style');
      ...
      [...this.els.el.querySelectorAll('[data-live-streaming-time]')].forEach((el) => {
        let isLive = false;

        JSON.parse(el.dataset.liveStreamingTime).some((dates) => {
          const timeStatus = this.utils.getTimeStatus(dates.start, dates.end);
          if (timeStatus === 'live') {
            isLive = true;
            return true;
          } else {
      ...
      [...this.els.layerPopupOpenButtonEls].forEach((el) => {
        el.addEventListener('click', () => {
          this.utils.setMobileFocusLoop(this.els.layerPopupWrapEl);
      ...
      if (this.els.beforeLoginButtonEl) {
        if (this.currentDevice === 'desktop') {
          this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
          this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-haspopup', 'true');
          this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
          this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-haspopup', 'true');
        }

        this.els.beforeLoginButtonEl.addEventListener('mouseenter', () => {
          if (this.currentDevice === 'desktop') {
            this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.add('show');
            this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'true');
          }
        });

        this.els.beforeLoginButtonEl.addEventListener('mouseleave', () => {
          if (this.currentDevice === 'desktop') {
            this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
            this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
          }
        });

        this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').addEventListener('click', (e) => {
          if (this.currentDevice === 'desktop') {
            e.preventDefault();
          }
        });

        this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').addEventListener('keydown', (e) => {
          if (this.currentDevice === 'desktop') {
            if (e.keyCode === KEY_CODE.ENTER || e.keyCode === KEY_CODE.DOWN || e.keyCode === KEY_CODE.SPACE) {
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.add('show');
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'true');
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list a').focus();
              e.preventDefault();
            }
          }
        });

        [...this.els.beforeLoginButtonEl.querySelectorAll('.nv00-gnb__utility-user-menu-link')].forEach((el) => {
          el.addEventListener('keydown', (e) => {
            if (e.keyCode === KEY_CODE.RIGHT || e.keyCode === KEY_CODE.DOWN) {
              let activeNextUserMenu = false;
              let nextUserMenu = el.parentElement.nextElementSibling;

              while (nextUserMenu) {
                if (!nextUserMenu.classList.contains('mobile-only')) {
                  activeNextUserMenu = nextUserMenu;
                  break;
                }

                nextUserMenu = nextUserMenu.nextElementSibling;
              }

              if (activeNextUserMenu) {
                activeNextUserMenu.querySelector('.nv00-gnb__utility-user-menu-link').focus();
              }
              e.preventDefault();
            } else if (e.keyCode === KEY_CODE.LEFT || e.keyCode === KEY_CODE.UP) {
              let activePrevUserMenu = false;
              let prevUserMenu = el.parentElement.previousElementSibling;

              while (prevUserMenu) {
                if (!prevUserMenu.classList.contains('mobile-only')) {
                  activePrevUserMenu = prevUserMenu;
                  break;
                }

                prevUserMenu = prevUserMenu.previousElementSibling;
              }

              if (activePrevUserMenu) {
                activePrevUserMenu.querySelector('.nv00-gnb__utility-user-menu-link').focus();
              }
              e.preventDefault();
            }
          });
        ...
        this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu:first-of-type .nv00-gnb__utility-user-menu-link').addEventListener('blur', () => {
          requestAnimationFrame(() => {
            if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__utility-user-menu-link')) {
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
            }
          });
        ...
        this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu:last-of-type .nv00-gnb__utility-user-menu-link').addEventListener('blur', () => {
          requestAnimationFrame(() => {
            if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__utility-user-menu-link')) {
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
        ...
        this.els.afterLoginButtonEl.addEventListener('mouseenter', () => {
          if (this.currentDevice === 'desktop') {
            this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.add('show');
            this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'true');
          }
        });

        this.els.afterLoginButtonEl.addEventListener('mouseleave', () => {
          if (this.currentDevice === 'desktop') {
            this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
            this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
          }
        });

        this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').addEventListener('click', (e) => {
          if (this.currentDevice === 'desktop') {
            e.preventDefault();
          }
        });

        this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').addEventListener('keydown', (e) => {
          if (this.currentDevice === 'desktop') {
            if (e.keyCode === KEY_CODE.ENTER || e.keyCode === KEY_CODE.DOWN || e.keyCode === KEY_CODE.SPACE) {
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.add('show');
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'true');
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list a').focus();
              e.preventDefault();
            }
          }
        });

        [...this.els.afterLoginButtonEl.querySelectorAll('.nv00-gnb__utility-user-menu-link')].forEach((el) => {
          el.addEventListener('keydown', (e) => {
            if (e.keyCode === KEY_CODE.RIGHT || e.keyCode === KEY_CODE.DOWN) {
              let activeNextUserMenu = false;
              let nextUserMenu = el.parentElement.nextElementSibling;
        ...
        this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu:first-of-type .nv00-gnb__utility-user-menu-link').addEventListener('blur', () => {
          requestAnimationFrame(() => {
            if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__utility-user-menu-link')) {
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
        ...
        this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu:last-of-type .nv00-gnb__utility-user-menu-link').addEventListener('blur', () => {
          requestAnimationFrame(() => {
            if (this.currentDevice === 'desktop' && !document.activeElement.classList.contains('nv00-gnb__utility-user-menu-link')) {
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
      ...
      this.els.innerWrapLoopingStartEl.addEventListener('focus', () => {
        if (this.els.backwardButtonEl.classList.contains('show')) {
          this.els.backwardButtonEl.focus();
        } else {
          if (this.els.beforeLoginButtonEl) {
            if (this.els.beforeLoginButtonEl.classList.contains('hide')) {
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu:last-of-type .nv00-gnb__utility-user-menu-link').focus();
            } else {
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu:last-of-type .nv00-gnb__utility-user-menu-link').focus();
            }
          } else {
            this.els.searchButtonEl.focus();
          }
      ...
      this.els.innerWrapLoopingEndEl.addEventListener('focus', () => {
        if (this.els.backwardButtonEl.classList.contains('show')) {
          this.els.backwardButtonEl.focus();
        } else {
          this.els.searchButtonEl.focus();
      ...
      document.addEventListener('scroll', () => {
        if (this.currentDevice === 'desktop' && this.els.dimWrapEl.classList.contains('show')) {
          this.els.dimWrapEl.style.top = `${this.els.el.getBoundingClientRect().bottom}px`;
          if (this.els.el.querySelector('.nv00-gnb__l1-menu-container.show.left-side, .nv00-gnb__l1-menu-container.show.right-side')) {
            this.els.el.querySelector('.nv00-gnb__l1-menu-container.show.left-side, .nv00-gnb__l1-menu-container.show.right-side').style.top = `${this.els.el.getBoundingClientRect().bottom - 24}px`;
          }
        }
      });

      window.addEventListener('mousewheel', (e) => {
        if (this.currentDevice === 'desktop' && this.els.el.querySelector('.nv00-gnb__l1-menu-container.show')) {
          requestAnimationFrame(() => {
            if (e.clientY > this.els.el.querySelector('.nv00-gnb__l1-menu-container.show').getBoundingClientRect().bottom) {
              this.l1MenuHideEventForDesktop();
            }
          });
        }
          ...
          if (this.getCurrentDevice() === 'desktop') {
            ...
            if (this.isSwiperInit) {
              ...
              [...this.els.el.querySelectorAll('.nv00-gnb__featured-products-thumbnail-item')].forEach((item) => {
                ...
                this.removeAllEventListeners(item.querySelector('.nv00-gnb__featured-products-thumbnail-item-link'), 'focus');
              });
            ...
            [...this.els.l0MenuEls].forEach((el) => {
              ...
              if (el.querySelector('.nv00-gnb__l0-menu-btn')) {
                ...
                [...el.querySelectorAll('.nv00-gnb__l1-menu-btn')].forEach((l1) => {
                  ...
                  l1.removeEventListener('keydown', this.l2MenuShowEventForMobile);
                });
            ...
            [...this.els.l0MenuEls].forEach((el) => {
              if (el.querySelector('.nv00-gnb__l0-menu-btn')) {
                ...
                el.querySelector('.nv00-gnb__l0-menu-btn').removeEventListener('keydown', this.l1MenuShowEventForDesktop);
            ...
            if (this.els.beforeLoginButtonEl) {
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').removeAttribute('aria-expanded');
              this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').removeAttribute('aria-haspopup');
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').removeAttribute('aria-expanded');
              this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').removeAttribute('aria-haspopup');
            }
          }
          this.currentDevice = this.getCurrentDevice();
        }
      ...
      window.addEventListener('keydown', (e) => {
        if (this.currentDevice === 'desktop' && e.keyCode === KEY_CODE.ESC) {
          if (this.els.el.querySelector('.nv00-gnb__l1-menu-container.show')) {
            this.els.el.querySelector('.nv00-gnb__l1-menu-container.show').previousElementSibling.focus();
            this.l1MenuHideEventForDesktop();
          }

          if (this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.contains('show')) {
            this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
            this.els.beforeLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
          }

          if (this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.contains('show')) {
            this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-user-menu-list').classList.remove('show');
            this.els.afterLoginButtonEl.querySelector('.nv00-gnb__utility-btn').setAttribute('aria-expanded', 'false');
          }
        }
  ...
  const nv00Gnb = {
    ...
    getCurrentDevice() {
      return Nv00Gnb.instances.get(document.querySelector('.nv00-gnb')).getCurrentDevice();
    },
  };
...
(function ($) {
	...
	ss.Sign = function () {
		...
		function setUserName(firstName, lastName, isLogin, isOtpLogin) {

			var userNameTxt = $.trim(firstName + " " + lastName);
			if(siteCode==="jp"){
				userNameTxt = $.trim(lastName + " " + firstName);
			}
...
//			profilePicture = "https://stg-account.samsung.com/membership/assets/images/layout/profile-icon-1.png";
			...
			if(isOtpLogin){
				profileLinkUrl = "/"+SITE_CD+"/web/mypage/profile-settings";
...
//
			...
			$(document).on("click",	userLoginBtn, function(event) {
				event.preventDefault();
				var loginEventFlag = false;
				if($(this).hasClass('nv00-gnb__utility-btn')){
					if(window.sg.components.nv00Gnb.getCurrentDevice() === "mobile"){
						loginEventFlag = true;
					}
				}else{
					loginEventFlag = true;
			...
			$(document).on("click",	accountBtn, function(event) {
				event.preventDefault();
				var accountEventFlag = false;
				if($(this).hasClass('nv00-gnb__utility-btn')){
					if(window.sg.components.nv00Gnb.getCurrentDevice() === "mobile"){
						accountEventFlag = true;
					}
				}else{
					accountEventFlag = true;
				}
				if(accountEventFlag){
					if(fnIsNull(profileLinkUrl)){
						window.location.href = "https://account.samsung.com/membership/contents/profile/profile-gate";
					}else{
						window.location.href = profileLinkUrl;
					}
				}
			});
